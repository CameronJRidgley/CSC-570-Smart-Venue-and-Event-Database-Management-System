...
| `vendor_sale` | `vendor_sales` | Team has a simpler shape (`sale_amount`, `sale_timestamp`). Ours adds `item_description`, `item_category`, `quantity`, `unit_price`, `payment_method`. The compat view exposes total_amount as sale_amount. |
